...
module.exports = {
  ...
  async runBlockMonitor(config) {
    ...
    for (const api in apis) {
      console.log(api);
      const current = await module.exports.getLatestBlocks(apis[api]);
      const current_height = current.height;
      const current_height_date = current.time;
      console.log("current_height_date", current_height_date);
      const current_height_delay =
        Math.abs(current_date - Date.parse(current_height_date)) /
        Math.pow(10, 3);
